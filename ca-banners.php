...
Description: Displays a banner with customizable scrolling message, settings, and an image.
Version: 1.1
Author: clientamp
...
function banner_plugin_register_settings() {
    register_setting('banner_plugin_settings', 'banner_plugin_settings');

    add_settings_section('banner_plugin_section', 'CA Banners Settings', null, 'banner-plugin');

    add_settings_field('banner_enabled', 'Enable Banner', 'banner_plugin_enabled_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_sitewide', 'Display Sitewide', 'banner_plugin_sitewide_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_message', 'Banner Message', 'banner_plugin_message_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_repeat', 'Number of Message Repeats', 'banner_plugin_repeat_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_background_color', 'Background Color', 'banner_plugin_background_color_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_text_color', 'Text Color', 'banner_plugin_text_color_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_font_size', 'Font Size', 'banner_plugin_font_size_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_font_family', 'Font Family', 'banner_plugin_font_family_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_border_width', 'Border Width (px)', 'banner_plugin_border_width_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_border_style', 'Border Style', 'banner_plugin_border_style_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_border_color', 'Border Color', 'banner_plugin_border_color_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_disable_mobile', 'Disable on Mobile', 'banner_plugin_disable_mobile_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_start_date', 'Start Date', 'banner_plugin_start_date_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_end_date', 'End Date', 'banner_plugin_end_date_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_urls', 'Display on Pages', 'banner_plugin_urls_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_exclude_urls', 'Exclude on Pages', 'banner_plugin_exclude_urls_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_image', 'Banner Image', 'banner_plugin_image_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_image_start_date', 'Image Start Date', 'banner_plugin_image_start_date_callback', 'banner-plugin', 'banner_plugin_section');
    add_settings_field('banner_image_end_date', 'Image End Date', 'banner_plugin_image_end_date_callback', 'banner-plugin', 'banner_plugin_section');
}
...
function banner_plugin_message_callback() {
    ...
    echo '<textarea name="banner_plugin_settings[message]" rows="4" cols="50">' . esc_textarea($message) . '</textarea>';
}
...
function banner_plugin_urls_callback() {
    ...
    echo '<textarea name="banner_plugin_settings[urls]" rows="4" cols="50">' . esc_textarea($urls) . '</textarea>';
    echo '<p class="description">Enter one URL per line to display the banner on specific pages.</p>';
}
...
function banner_plugin_sitewide_callback() {
    ...
    echo '<p class="description">Enable to display banner across all pages. This will override the "Display on Pages" setting.</p>';
}

function banner_plugin_exclude_urls_callback() {
    $settings = get_option('banner_plugin_settings');
    $exclude_urls = isset($settings['exclude_urls']) ? $settings['exclude_urls'] : '';
    echo '<textarea name="banner_plugin_settings[exclude_urls]" rows="4" cols="50">' . esc_textarea($exclude_urls) . '</textarea>';
    echo '<p class="description">Enter one URL per line to exclude the banner from specific pages when sitewide is enabled.</p>';
}
...
function banner_plugin_enqueue_scripts() {
   ...
   // Update: Get current URL without query parameters and ensure it has a trailing slash
    ...
    echo '<script>';
    echo 'var bannerMessage = \'' . addslashes($message) . ' &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; \';';
    ...
    */
    ...
    <?php
    echo '</script>';
    echo '<style>html{ margin-top:6rem !important;} .banner-button { display: inline-block; padding: 5px 12px; background-color: rgb(206, 122, 49);
        color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 600; text-decoration: none; transition: all 0.3s ease; } .banner-button:hover, .banner-button:focus { background-color: rgba(206, 122, 49, 0.8); color: white; text-decoration: none}</style>';
}
